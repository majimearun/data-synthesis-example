...
def find_most_least_frequent_values(df, column1, column2):
    ...
    final_df = final_df.append(result_df.head(5))
...
for column_name in columns_names:
    ...
    with open(f"./insights/{column_name}.txt", "w") as file:
        ...
        if column.dtype == "int64" or column.dtype == "float64":
            ...
            print("1. Date-based aggregations:")

            object_columns = data.select_dtypes(include=["object"]).columns
            numeric_columns = data.select_dtypes(include=["int64", "float64"]).columns

            for other_column in object_columns:
                print(f"checking frequency of {other_column} yearly, monthly, daily: ")

                print("   - Number of records per day:")

                daily_counts: pd.DataFrame = data.groupby(column.dt.date)[
                    other_column
                ].nunique()

                daily_counts.sort_values(inplace=True, ascending=False)
                daily_counts = daily_counts.head(5).copy()
                ...
                monthly_counts: pd.DataFrame = data.groupby(column.dt.to_period("M"))[
                    other_column
                ].nunique()

                monthly_counts.sort_values(inplace=True, ascending=False)
                monthly_counts = monthly_counts.head(5).copy()
                ...
                yearly_counts.sort_values(inplace=True, ascending=False)
                yearly_counts = yearly_counts.head(5).copy()
                yearly_counts: pd.DataFrame = data.groupby(column.dt.to_period("Y"))[
                    other_column
                ].nunique()
